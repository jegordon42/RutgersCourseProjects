...
void sortCircuit(){
	...
	for(i = 0; i < numVariables; i++)
	{
		...
		if(curVar->isInput == FALSE)
			curVar->isDefined = FALSE;
		else
			curVar->isDefined = TRUE;
	}
	...
	for(i = 0; i < numGates; i++)
	{
		...
		if(strcmp(curGate->type, "NOT") == 0)
		{
			...
			if(curVar->isDefined == FALSE)
			{
					putAfterDefinition(curGate, curVar);
					return;
			}
		...
		if(strcmp(curGate->type, "DECODER") == 0)
		{
			...
			{
				curVar = getVariable(*(curGate->varIndices + j));
				if(curVar->isDefined == FALSE)
				{
					putAfterDefinition(curGate, curVar);
					return;
			...
			for(j = 0; j < toThePower(2, curGate->numInputs); j++)
			{
					curVar = getVariable(*(curGate->varIndices +curGate-> numInputs + j));
					curVar->isDefined = TRUE;
			}
		}
		if(strcmp(curGate->type, "MULTIPLEXER") == 0)
		{
			for(j = 0; j < curGate->numInputs + logTwo(curGate->numInputs) ; j++)
			{
				curVar = getVariable(*(curGate->varIndices + j));
				if(curVar->isDefined == FALSE)
				{
			...
			curVar = getVariable(*(curGate->varIndices + (curGate->numInputs + logTwo(curGate->numInputs))));
			curVar->isDefined = TRUE;
		}
	}
}
...
void putAfterDefinition(gateNode* gateToSwitch, variableNode* undefinedVar){
	...
	for(i = 0; i < numGates; i++)
	{
		...
		{
			curVar = getVariable(*(curGate->varIndices + 1));
			if(curVar->index == undefinedVar->index)
				break;
		}
		if(strcmp(curGate->type, "AND") == 0 || strcmp(curGate->type, "OR") == 0 || strcmp(curGate->type, "NAND") == 0 || strcmp(curGate->type, "NOR") == 0 || strcmp(curGate->type, "XOR") == 0)
		{
	...
	int tempIndex = curGate->index;
	curGate->index = gateToSwitch->index;
	gateToSwitch->index = tempIndex;
	//printf("%d - %d\n", gateToSwitch->index, curGate->index); 
	sortCircuit();
}
	...
	for(x = 0; x < toThePower(2, numInputs); x++)
	{
		...
		for(y = 0; y < numGates; y++)
		{
			...
			if(strcmp(curGate->type, "OR") == 0)
			{
				curVar = getVariable(*(curGate->varIndices));
				int val1 = curVar->value;
				//printf("%s -%d OR ",curVar->name, val1 );
				curVar = getVariable(*(curGate->varIndices + 1));
				int val2 = curVar->value;
				//printf("%s -%d = ",curVar->name, val2 );
				curVar = getVariable(*(curGate->varIndices + 2));
				if(val1 == 1 || val2 == 1)
					curVar->value = 1;
				else
					curVar->value = 0;
				//printf("%s - %d\n",curVar->name, curVar->value );
			}
			...
			if(strcmp(word, "INPUTVAR") == 0 || strcmp(word, "OUTPUTVAR") == 0)
			{
					...
					while(line[lineIndex] != '\n')
					{
						...
						if(word[0] >= 'A' && word[0] <= 'Z')
						{
							...
							if(isInput == TRUE)
								newVariable->isInput = TRUE;
							else
								newVariable->isInput = FALSE;
